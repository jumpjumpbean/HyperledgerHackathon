...
<div class="ncc-receipt-info">
    ...
    <table class="ncc-table-style">
        ...
        <tr>
            ...
            <th class="w80"><?php echo $lang['cart_index_trade_type'];?></th>
        ...
        <?php foreach($output['store_cart_list_trade'] as $store_id => $cart_list) {?>
        <tbody>
        <tr>
            <th colspan="20"><strong>店铺：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id));?>"><?php echo $cart_list[key($cart_list)][0]['store_name']; ?></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
                <div class="store-sale">
                    ...
                    <?php if (!empty($output['store_mansong_rule_list'][$store_id])) {?>
                        <em><i class="icon-gift"></i>店铺活动-满减</em><?php echo $output['store_mansong_rule_list'][$store_id]['desc'];?>
        ...
        <!--S trade type-->
        <?php foreach($cart_list as $trade_id => $cart_info_trade) {?>

            <?php foreach($cart_info_trade as $cart_info) {?>
                <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
                    ...
                    <td class="w80">
                        <em>
                            <!--tom modify
            <?php/* if (!empty($output['trade_type'])) {
                    echo $output['trade_type'];
                  } else {
                      echo $output['trade_type_arr'][$cart_info['trade_type_id']];
                    }*/?>
            -->
                            <?php echo $output['trade_type_arr'][$trade_id];?>
                        </em>
                    </td>
            ...
            <?php }?>
            <!--trade type parting line-->
            <tr colspan="20"><td colspan="20"><hr /></td></tr>
            <!--E trade type-->
        ...
        <tr style="display: none">
            <td class="tr" colspan="20"><div class="ncc-store-account">
                    ...
                    <?php if (!empty($output['store_mansong_rule_list'][$store_id]['discount'])) {?>
                        <dl class="mansong">
                            <dt>满减活动-<?php echo $output['store_mansong_rule_list'][$store_id]['desc'];?>：</dt>
                    ...
                    <?php if (!empty($output['store_voucher_list'][$store_id]) && is_array($output['store_voucher_list'][$store_id]) && $output['trade_type_id'] != 4) {?>
                        <dl class="voucher">
                            <dt>
        ...
        <tfoot>
        <tr>
            <td colspan="20"><div class="ncc-each-account ">运费：<em id="freightTotal"></em><?php echo $lang['currency_zh'];?></div></td>
        </tr>

        <tr>
            <td colspan="20"><div class="ncc-each-account">商品金额：<em id="goodsTotal"></em><?php echo $lang['currency_zh'];?></div></td>
        </tr>
        <tr>
            <td colspan="20"><div class="ncc-each-account">海关税费：<em id="taxTotal"><?php echo $output['directmail_tax_total']?></em><?php echo $lang['currency_zh'];?></div></td>
        </tr>
        </tfoot>
    </table>
    ...
    <?php if (!empty($output['available_pd_amount']) || !empty($output['available_rcb_amount'])) { ?>
        <tr id="pd_panel">
            <td class="pd-account" colspan="20"><div class="ncc-pd-account">
                    <div class="ncc-receipt-info-title" style="float:left">
                        <h3>积分抵扣</h3>
                    </div>
                    <?php if (!empty($output['available_pd_amount'])) { ?>
                        <div class="mt5 mb5">
                            <label>
                                ...
                                <input type="checkbox" class="vm mr5" value="1" name="pd_pay">
                                酒知道商城积分:<?php echo $output['gradevin'];?>分（可抵金额：<em id="points"><?php echo $output['gradevin']*0.01;?></em><?php echo $lang['currency_zh'];?>）</label>
                        </div>
                        <div class="mt5 mb5">
                            <label>
                                    <input type="checkbox" class="vm mr5" value="2" name="pd_pay1">
                                    港韩购商城积分:<?php echo $output['normandy'];?>分（可抵金额：<em id="points1"><?php echo $output['normandy']*0.01;?></em><?php echo $lang['currency_zh'];?>）</label>
                        </div>
                    <?php } ?>
