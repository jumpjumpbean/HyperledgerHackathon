...
class member_buyControl extends mobileMemberControl {
	...
	if($member_info['is_buy'] ==0){
		output_error('您没有商品购买的权限');
	}
    }
    ...
    public function buy_step1Op() {
        ...
        $cart_id=urldecode($_POST['cart_id']);
        ...
        if(isset($_POST['from'])&& $_POST['from']=='android'){
            $iscart = $_POST['iscart'];
            $result = $logic_buy->buyAndroidStep1($iscart,$cart_id, $_POST['ifcart'], $this->member_info['member_id'], $this->member_info['store_id'],null,$member_discount,$member_level);
        }else{
        ...
        //若收货姓名大于5个字，则省略显示
        if(iconv_strlen(($buy_list['address_info']['true_name']))>=5){
            $buy_list['address_info']['true_name'] = str_cut($buy_list['address_info']['true_name'],8).'...';
        }
        $buy_list['ifshow_offpay'] = $result['ifshow_offpay'];
        //添加重写的cart_id
        $buy_list['cart_id_create'] = $result['cart_id_create'];
        ...
        $buy_list['buyer_role'] = $member_info['member_role'];
        $buy_list['cart_id_create']=
    ...
    public function buy_step2Op() {
        ...
        $param['alias'] = $_POST['alias'];
        $param['rpt'] = $_POST['rpt'];
        $param['inviter_id']=!empty($_POST['inviter_id'])? $_POST['inviter_id']:'';
        $param['tp_id']=!empty($_POST['tp_id'])? $_POST['tp_id']:'';
        ...
        $param['order_from'] = intval($_POST['order_from'])!=0 ? intval($_POST['order_from']):2;
        $logic_buy = logic('buy');
        ...
        $param['buyer_role'] = $member_info['member_role'];
        ...
        output_data(array('pay_sn' => $result['data']['pay_sn'],'payment_code'=>$order_info['payment_code'],'buyer_role'=>$member_info['member_role']));
    }
